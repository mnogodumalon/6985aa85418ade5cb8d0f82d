...
interface VerfuegbarkeitDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Verfuegbarkeit['fields']) => Promise<void>;
  defaultValues?: Verfuegbarkeit['fields'];
  mitarbeiterList: Mitarbeiter[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function VerfuegbarkeitDialog({ open, onClose, onSubmit, defaultValues, mitarbeiterList, enablePhotoScan = true, enablePhotoLocation = true }: VerfuegbarkeitDialogProps) {
  const [fields, setFields] = useState<Partial<Verfuegbarkeit['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'verfuegbarkeit');
      await onSubmit(clean as Verfuegbarkeit['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "mitarbeiter": string | null, // Display name from Mitarbeiter (see <available-records>)\n  "von_datum": string | null, // YYYY-MM-DD\n  "bis_datum": string | null, // YYYY-MM-DD\n  "verfuegbarkeitsstatus": LookupValue | null, // Status (select one key: "eingeschraenkt_verfuegbar" | "verfuegbar" | "nicht_verfuegbar") mapping: eingeschraenkt_verfuegbar=Eingeschränkt verfügbar, verfuegbar=Verfügbar, nicht_verfuegbar=Nicht verfügbar\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter"]);
        ...
        return merged as Partial<Verfuegbarkeit['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Verfügbarkeit bearbeiten' : 'Verfügbarkeit hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="von_datum">Verfügbar von</Label>
            <Input
              id="von_datum"
              type="date"
              value={fields.von_datum ?? ''}
              onChange={e => setFields(f => ({ ...f, von_datum: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="bis_datum">Verfügbar bis</Label>
            <Input
              id="bis_datum"
              type="date"
              value={fields.bis_datum ?? ''}
              onChange={e => setFields(f => ({ ...f, bis_datum: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="verfuegbarkeitsstatus">Status</Label>
            <Select
              value={lookupKey(fields.verfuegbarkeitsstatus) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, verfuegbarkeitsstatus: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="verfuegbarkeitsstatus"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="eingeschraenkt_verfuegbar">Eingeschränkt verfügbar</SelectItem>
