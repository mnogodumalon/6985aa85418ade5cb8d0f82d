...
import { VerfuegbarkeitViewDialog } from '@/components/dialogs/VerfuegbarkeitViewDialog';
...
import { SchichtzuweisungenViewDialog } from '@/components/dialogs/SchichtzuweisungenViewDialog';
...
const VERFUEGBARKEIT_FIELDS = [
  { key: 'mitarbeiter', label: 'Mitarbeiter', type: 'applookup/select', targetEntity: 'mitarbeiter', targetAppId: 'MITARBEITER', displayField: 'mitarbeiternummer' },
  { key: 'von_datum', label: 'Verfügbar von', type: 'date/date' },
  { key: 'bis_datum', label: 'Verfügbar bis', type: 'date/date' },
  { key: 'verfuegbarkeitsstatus', label: 'Status', type: 'lookup/select', options: [{ key: 'eingeschraenkt_verfuegbar', label: 'Eingeschränkt verfügbar' }, { key: 'verfuegbar', label: 'Verfügbar' }, { key: 'nicht_verfuegbar', label: 'Nicht verfügbar' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const MITARBEITER_FIELDS = [
  { key: 'mitarbeiternummer', label: 'Mitarbeiternummer', type: 'string/text' },
...
const SCHICHTZUWEISUNGEN_FIELDS = [
  { key: 'mitarbeiter', label: 'Mitarbeiter', type: 'applookup/select', targetEntity: 'mitarbeiter', targetAppId: 'MITARBEITER', displayField: 'mitarbeiternummer' },
  { key: 'schichtvorlage', label: 'Schichtvorlage', type: 'applookup/select', targetEntity: 'schichtvorlagen', targetAppId: 'SCHICHTVORLAGEN', displayField: 'endzeit' },
  { key: 'datum', label: 'Datum', type: 'date/date' },
  { key: 'tatsaechliche_startzeit', label: 'Tatsächliche Startzeit', type: 'string/text' },
  { key: 'tatsaechliche_endzeit', label: 'Tatsächliche Endzeit', type: 'string/text' },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];
const SCHICHTVORLAGEN_FIELDS = [
  { key: 'endzeit', label: 'Endzeit', type: 'string/text' },
...
const ENTITY_TABS = [
  { key: 'verfuegbarkeit', label: 'Verfügbarkeit', pascal: 'Verfuegbarkeit' },
  { key: 'mitarbeiter', label: 'Mitarbeiter', pascal: 'Mitarbeiter' },
  { key: 'schichtzuweisungen', label: 'Schichtzuweisungen', pascal: 'Schichtzuweisungen' },
  { key: 'schichtvorlagen', label: 'Schichtvorlagen', pascal: 'Schichtvorlagen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('verfuegbarkeit');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'verfuegbarkeit': new Set(),
    'mitarbeiter': new Set(),
    'schichtzuweisungen': new Set(),
    'schichtvorlagen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'verfuegbarkeit': return (data as any).verfuegbarkeit as Verfuegbarkeit[] ?? [];
      case 'mitarbeiter': return (data as any).mitarbeiter as Mitarbeiter[] ?? [];
      case 'schichtzuweisungen': return (data as any).schichtzuweisungen as Schichtzuweisungen[] ?? [];
      case 'schichtvorlagen': return (data as any).schichtvorlagen as Schichtvorlagen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'verfuegbarkeit':
        ...
        break;
      case 'schichtzuweisungen':
        lists.mitarbeiterList = (data as any).mitarbeiter ?? [];
        lists.schichtvorlagenList = (data as any).schichtvorlagen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'verfuegbarkeit' && fieldKey === 'mitarbeiter') {
      const match = (lists.mitarbeiterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.mitarbeiternummer ?? '—';
    }
    if (entity === 'schichtzuweisungen' && fieldKey === 'mitarbeiter') {
      const match = (lists.mitarbeiterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.mitarbeiternummer ?? '—';
    }
    if (entity === 'schichtzuweisungen' && fieldKey === 'schichtvorlage') {
      const match = (lists.schichtvorlagenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.endzeit ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'verfuegbarkeit': return VERFUEGBARKEIT_FIELDS;
      case 'mitarbeiter': return MITARBEITER_FIELDS;
      case 'schichtzuweisungen': return SCHICHTZUWEISUNGEN_FIELDS;
      case 'schichtvorlagen': return SCHICHTVORLAGEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'verfuegbarkeit': return {
        create: (fields: any) => LivingAppsService.createVerfuegbarkeitEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVerfuegbarkeitEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVerfuegbarkeitEntry(id),
      };
      ...
      case 'schichtzuweisungen': return {
        create: (fields: any) => LivingAppsService.createSchichtzuweisungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtzuweisungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtzuweisungenEntry(id),
      };
      case 'schichtvorlagen': return {
        create: (fields: any) => LivingAppsService.createSchichtvorlagenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtvorlagenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtvorlagenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'verfuegbarkeit' || dialogState?.entity === 'verfuegbarkeit') && (
        <VerfuegbarkeitDialog
          open={createEntity === 'verfuegbarkeit' || dialogState?.entity === 'verfuegbarkeit'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'verfuegbarkeit' ? handleUpdate : (fields: any) => handleCreate('verfuegbarkeit', fields)}
          defaultValues={dialogState?.entity === 'verfuegbarkeit' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'schichtzuweisungen' || dialogState?.entity === 'schichtzuweisungen') && (
        <SchichtzuweisungenDialog
          open={createEntity === 'schichtzuweisungen' || dialogState?.entity === 'schichtzuweisungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtzuweisungen' ? handleUpdate : (fields: any) => handleCreate('schichtzuweisungen', fields)}
          defaultValues={dialogState?.entity === 'schichtzuweisungen' ? dialogState.record?.fields : undefined}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
          schichtvorlagenList={(data as any).schichtvorlagen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Schichtzuweisungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Schichtzuweisungen']}
        />
      )}
      {(createEntity === 'schichtvorlagen' || dialogState?.entity === 'schichtvorlagen') && (
        <SchichtvorlagenDialog
          open={createEntity === 'schichtvorlagen' || dialogState?.entity === 'schichtvorlagen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtvorlagen' ? handleUpdate : (fields: any) => handleCreate('schichtvorlagen', fields)}
          defaultValues={dialogState?.entity === 'schichtvorlagen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Schichtvorlagen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Schichtvorlagen']}
        />
      )}
      {viewState?.entity === 'verfuegbarkeit' && (
        <VerfuegbarkeitViewDialog
          open={viewState?.entity === 'verfuegbarkeit'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'verfuegbarkeit', record: r }); }}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
        />
      )}
      ...
      {viewState?.entity === 'schichtzuweisungen' && (
        <SchichtzuweisungenViewDialog
          open={viewState?.entity === 'schichtzuweisungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtzuweisungen', record: r }); }}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
          schichtvorlagenList={(data as any).schichtvorlagen ?? []}
        />
      )}
      {viewState?.entity === 'schichtvorlagen' && (
        <SchichtvorlagenViewDialog
          open={viewState?.entity === 'schichtvorlagen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtvorlagen', record: r }); }}
        />
      )}
